...
While it interfaces with the CPU-based ALE under the hood, it exposes a purely JAX-compatible API so you can trace, compile, and batch your environments effortlessly.
...
## Sharp Bits 🔪
Due to the nature of wrapping a CPU-based emulator in callbacks, there are a few limitations to keep in mind.
...
## Installation
`alenax` is available on PyPI and can be installed via pip.
```bash
pip install alenax
```
...
In order to properly use the `RecordEpisodeStatistics` wrapper, make sure to initialize the environment with `clip_rewards=False` and to apply the wrapper before `EpisodicLife` and `ClipReward`.

```python
from alenax import AtariEnv, ClipReward, EpisodicLife, RecordEpisodeStatistics

env = AtariEnv("pong", clip_rewards=False)
